    ...
    *((uint32_t*)prg.data.data()) = header; 
    ...
    if(elfIo.load(algorithmPath))
    {
        ...
        for(const auto& section : elfIo.sections)
        {
            ...
            if(secName == std::string("DevDscr"))
            {
                const LoaderDescription& desc = *reinterpret_cast<LoaderDescription const*>(section->get_data());
                lut.loaderDesc = desc;
                ESP_LOGI(TAG, "Flash Loader Version %x", lut.loaderDesc.version);
                ESP_LOGI(TAG, "Flash Loader Name %s", lut.loaderDesc.devName);
                ESP_LOGI(TAG, "Flash Loader Type %u", lut.loaderDesc.devType);
                ESP_LOGI(TAG, "Flash Addr %lx", lut.loaderDesc.DevAdr);
                ESP_LOGI(TAG, "Flash Size %lx", lut.loaderDesc.szDev);
            }
        }
#endif
    }
...
{
    ESP_LOGI(TAG, "%s addr 0x%lX", __func__, adr);
